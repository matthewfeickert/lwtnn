...
# Conversion routine
SPLIT=../scripts/lwtnn-split-keras-network.py
...
echo " == Keras merged -> keras split -> json -> lwtnn unit test ======="
...
(
    ...
    wget -nv $INPUT
    tar xf ${INPUT##*/}
    if [[ ! -f $H5_MODEL || ! -f $VARIABLES ]] ; then
...
echo " -- Splitting network --"
$SPLIT $TMPDIR/$H5_MODEL -w $TMPDIR/$HDF5 -a $TMPDIR/$ARCH
